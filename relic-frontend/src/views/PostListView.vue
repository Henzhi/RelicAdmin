    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">用户动态管理</span>
        </div>
      </template>
      ...
      <el-table :data="tableData" v-loading="loading" stripe border>
        ...
        <el-table-column prop="artifactId" label="关联文物" width="90" />
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="{ row }">
            ...
            <el-popconfirm title="确定要删除该动态吗？" @confirm="handleDelete(row)">
              <template #reference>
...
import { getPostPage, approvePost, rejectPost, deletePost } from '../api/postAdmin'
...
async function fetchData() {
  ...
  } catch { ElMessage.error('加载动态列表失败') }
...
async function handleApprove(row) { try { await approvePost(row.id); row.status = 'approved'; ElMessage.success('已通过') } catch { ElMessage.error('操作失败') } }

function handleRejectOpen(row) { currentRejectId = row.id; rejectReason.value = ''; rejectDialogVisible.value = true }

async function handleRejectConfirm() {
  if (!rejectReason.value) { ElMessage.warning('请输入拒绝原因'); return }
  try { await rejectPost(currentRejectId, rejectReason.value); rejectDialogVisible.value = false; fetchData(); ElMessage.success('已拒绝') }
  catch { ElMessage.error('操作失败') }
}

async function handleDelete(row) {
  try { await deletePost(row.id); ElMessage.success('删除成功'); fetchData() }
